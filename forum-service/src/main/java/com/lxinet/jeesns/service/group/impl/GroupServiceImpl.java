...
public class GroupServiceImpl implements IGroupService {
    ...
    @Override
    public ResponseModel listByPage(int status, Page page, String key, int memberId, int type) {
        if (StringUtils.isNotBlank(key)){
            key = "%"+key.trim()+"%";
        }
        List<Group> list = groupDao.listByPage(page, status, key, memberId, type);
    ...
    public ResponseModel findDetail(int id) {
        return new ResponseModel(0, "获取成功",  findById(id));
    }
    ...
    public ResponseModel save(Member loginMember,Group group) {
        ...
        if(loginMember.getIsAdmin() > 0){
            ...
            if("0".equals(config.get(ConfigUtil.GROUP_APPLY))){
                return new ResponseModel(-1,"社团申请功能已关闭");
            }
