...
public class GroupTopicServiceImpl implements IGroupTopicService {
        ...
        if(archiveService.save(member,archive)){
            ...
            if(result == 1){
                ...
                if (groupTopic.getStatus() == 0){
                    return new ResponseModel(2,"活动发布成功，请等待管理员审核通过","../detail/"+groupTopic.getGroupId());
                }

                return new ResponseModel(2,"活动发布成功","../topic/"+groupTopic.getId());
            }
        }
        return new ResponseModel(-1,"活动发布失败");
    }
    ...
    public ResponseModel delete(Member loginMember,int id) {
        ...
        if(groupTopic == null){
            return new ResponseModel(-1,"活动不存在");
        }
